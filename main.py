...
if os.path.exists('pas.json'):
    ...
    if val.status_code == 403:
        ...
        if ref.status_code == 403:
            print('Старая сессия не найдена в базе данных (скорее всего сервер был перезагружен)\nНеобходим повторный вход')
            user = auth()
        else:
